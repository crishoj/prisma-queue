...
var debug = (0, import_debug.default)("prisma-queue");
...
var PrismaQueue = class extends import_events.EventEmitter {
  /**
  ...
  constructor(options, worker) {
    ...
    (0, import_node_assert.default)(name.length <= 255, "name must be less or equal to 255 chars");
    (0, import_node_assert.default)(pollInterval >= 100, "pollInterval must be more than 100 ms");
  ...
  async schedule(options, payloadOrFunction) {
    ...
    const runAt = firstRunAt ?? new import_croner.Cron(cron).nextRun();
    (0, import_node_assert.default)(runAt, `Failed to find a future occurrence for given cron`);
  ...
  async processJob(job) {
    ...
    try {
      (0, import_node_assert.default)(this.worker, "Missing queue worker to process job");
